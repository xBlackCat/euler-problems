package org.xblackcat.euler.problem11;
 ...
 */
@InputData({})
@ResultDescription("The greatest product of four adjacent numbers in the same direction: {0,number,#}")
    ...
    public long product() {
        ...
        for (int[] row : BLOCK) {
            for (int j = 3; j < width; j++) {
                if (row[j - 3] != 0 && row[j - 2] != 0 && row[j - 1] != 0 && row[j] != 0) {
                    series.add(new Series(row[j - 3], row[j - 2], row[j - 1], row[j]));
                }
            }
        }
        for (int i = 3; i < height; i++) {
            for (int j = 0; j < width; j++) {
                if (BLOCK[i - 3][j] != 0 && BLOCK[i - 2][j] != 0 && BLOCK[i - 1][j] != 0 && BLOCK[i][j] != 0) {
                    series.add(new Series(BLOCK[i - 3][j], BLOCK[i - 2][j], BLOCK[i - 1][j], BLOCK[i][j]));
                }
            }
        }
        for (int i = 3; i < height; i++) {
            for (int j = 3; j < width; j++) {
                if (BLOCK[i - 3][j - 3] != 0 && BLOCK[i - 2][j - 2] != 0 && BLOCK[i - 1][j - 1] != 0 && BLOCK[i][j] != 0) {
                    series.add(new Series(BLOCK[i - 3][j - 3], BLOCK[i - 2][j - 2], BLOCK[i - 1][j - 1], BLOCK[i][j]));
                }
            }
        }
        for (int i = 3; i < height; i++) {
            for (int j = 3; j < width; j++) {
                if (BLOCK[i - 3][j] != 0 && BLOCK[i - 2][j - 1] != 0 && BLOCK[i - 1][j - 2] != 0 && BLOCK[i][j - 3] != 0) {
                    series.add(new Series(BLOCK[i - 3][j], BLOCK[i - 2][j - 1], BLOCK[i - 1][j - 2], BLOCK[i][j - 3]));
                }
            }
        }

        return series.stream().mapToLong(Series::multiply).max().orElse(-1);
    }
}
